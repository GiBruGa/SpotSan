...
- [ ] **Non fait, confirmé nécessaire par Gilles le 2026-08-22 (retour après premier essai réel)** : le bandeau de filtres du haut de v1 (famille de source — Gouv/données publiques (OSM)/SpotSan (PSM) — et filtres sur les prestations/équipements) n'existe plus du tout dans la carte V2 (un seul style de marqueur pour tous). Demande explicite d'ajouter en plus des filtres sur les prestations/équipements, **en particulier PMR et Enfant** (colonnes déjà en base : `PMR` et `Adapte_Enfant` sur `SanitaryBlocks_Inventory`). **Prochain chantier à ouvrir**, pas commencé.
