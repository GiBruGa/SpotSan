...
- [x] **Filtres de carte — fait et vérifié 2026-08-22**, port fidèle de v1 (`SpotSan/app.js` : `classify()`, `passesRefinements()`, `V_COLORS`/`V_LABELS`, chips HTML) plutôt qu'une réinvention :
  - Bouton "⚙ Filtres (n)" repliable, en haut à gauche de la carte (contrôle de zoom Leaflet déplacé en bas à gauche pour ne pas se chevaucher, comme en v1).
  - 5 chips de famille de source, avec compteurs : **Vérifiés** (bleu, Gouv+OSM), **Officiel .gouv** (orange), **Données publiques** (vert, OSM), **SpotSan** (rose, PSM), **Supprimées** (rouge-brun, masquée par défaut) — un groupe `markerClusterGroup` Leaflet par famille, classification identique à v1 (`classifier()` dans `src/lib/classification.js`, même ordre de priorité Certified avant Verified).
  - 3 chips d'affinage combinables : **PMR** (`PMR > 0`), **Enfant** (`Adapte_Enfant === true`), **4★ mini** (`Rating_Overall >= 4`, échelle héritée v1).
  - Marqueurs colorés par famille (cercles Leaflet, pas les icônes SVG détaillées de v1 avec badges/étoile — simplifié, la distinction par couleur suffit), plus estompés pour "Supprimées".
  - Les filtres se recalculent depuis les données déjà chargées (pas de nouvelle requête réseau à chaque bascule de chip) ; seul un déplacement de la carte redéclenche un chargement.
  - Vérifié dans le navigateur : les 5 familles + 3 affinages s'affichent avec les bons libellés/compteurs, bascule d'un chip change le compte affiché et les marqueurs visibles, clic sur un marqueur ouvre toujours la fiche (non-régression).
...
- 2026-08-31 — **Backlog noté (résolu depuis, voir entrée suivante) — menu Exploitant dans EkoMa pour corriger le statut d'un sanitaire.** Le système de consensus par avis concordants ("3 Usagers signalent Inexistant/Hors Service" → bascule automatique, cf. `vote_toilet_position`/`Toilet_Position_Votes` comme précédent direct pour ce pattern) a un angle mort identifié par Gilles : un sanitaire une fois marqué HS n'est plus visité, donc ne reçoit plus jamais assez d'avis "Disponible" pour rebasculer — la remise en état passe inaperçue côté carte. Solution envisagée : donner aux comptes **Exploitant** (rôle `parcs_sanitaires`/`contractors`, cf. mémoire "PointSan access tiers") un menu dédié dans EkoMa avec des actions directes sur le statut d'un sanitaire de leur parc : **"HS"**, **"Condamné"**, **"Supprimé"**, **"Remise en conformité"** — déclaratif, sans attendre de confirmations croisées, puisque l'exploitant est la personne qui *sait* quand c'est réparé. À construire après le système de vote côté SpotSan (Lot en cours).

- 2026-08-22 — **Filtres de carte livrés** : port fidèle de v1 (chips par famille de source + affinages PMR/Enfant/4★ mini), voir le détail au Lot 2 ci-dessus. Boucle les 3 retours du 2026-08-22.
- 2026-08-22 (session en autonomie, arrêtée volontairement à ce point — voir ci-dessous) — **3 retours de Gilles après premier essai réel de l'app, traités partiellement** :
  1. **Mise à jour automatique** — le mécanisme n'était pas vraiment câblé (juste un `navigator.serviceWorker.register`, aucune détection/activation d'une nouvelle version). **Corrigé** : bandeau "Nouvelle version disponible" avec bouton, via `virtual:pwa-register/svelte`. Volontairement **pas de rechargement automatique silencieux** — risque d'effacer une saisie en cours (formulaire/photo non sauvegardée) ; l'utilisateur choisit le moment. Vérifié check périodique (toutes les heures) mis en place.
  2. **Menu utilisateur** — diagnostic : **pas un bug** (le clic ouvrait déjà le menu, y compris en vue mobile, vérifié) — le menu était juste très pauvre (pseudo/téléphone en lecture seule + suppression de compte). **Étoffé avec tout ce qui était demandé** : "Mes informations" (édition complète : pseudo, téléphone, avatar, sexe, année, handicaps — nouvelle fonction `mettreAJourProfil`), "Installer l'application" (QR code généré localement, bibliothèque `qrcode`, aucun service tiers), bascule d'affichage Auto/Clair/Sombre (mécanique posée dans `theme.js` + tokens CSS dans `app.css`, persistée), "À propos" (slogan + mention légale reprise de la convention EkoMa/v1), "Se déconnecter" (nouveau, distinct de la suppression de compte). Tout testé de bout en bout dans le navigateur, aucune erreur.
     - **Couverture du thème clair/sombre partielle et assumée** : seuls le bandeau et ses sous-panneaux (menu, Mes informations, Installation, À propos) utilisent les nouveaux tokens CSS. Le reste de l'app (formulaire d'avis, fiche sanitaire, écran d'inscription, signalement I&V) garde ses couleurs fixes actuelles et ne change pas visuellement quand on bascule le thème — pas un bug (rien ne casse), juste incomplet. À reprendre pour une couverture complète.
  3. **Filtres de la carte** (famille de source v1 + PMR/Enfant) — **confirmé nécessaire, pas commencé**, voir la case ouverte au Lot 2 ci-dessus.
  - **Séance interrompue volontairement** : Gilles partait en voiture, cette session tourne en local sur sa machine (pas dans le cloud) — fermer/mettre en veille l'ordinateur interromprait le travail en cours. Sur sa demande explicite, arrêt propre après avoir terminé et vérifié le point 2 (menu), sans démarrer le point 3 (filtres carte) pour ne pas laisser un chantier à moitié fait.
